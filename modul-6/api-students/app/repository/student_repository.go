...
func (r *studentPostgresRepository) FindAll(ctx context.Context, q model.ListQuery) ([]model.Student, int, error) {
	...
	for rows.Next() {
		var s model.Student
		if err := rows.Scan(&s.ID, &s.NIM, &s.Name, &s.Grade, &s.Password, &s.Role, &s.OwnerID, &s.IsActive, &s.CreatedAt); err != nil {
...
func (r *studentPostgresRepository) FindByID(ctx context.Context, id int) (model.Student, error) {
	...
	).Scan(&s.ID, &s.NIM, &s.Name, &s.Grade, &s.Password, &s.Role, &s.OwnerID, &s.IsActive, &s.CreatedAt)
...
func (r *studentPostgresRepository) Create(ctx context.Context, s model.Student) (model.Student, error) {
	...
	err := r.pool.QueryRow(ctx,
		...
		s.NIM, s.Name, s.Grade, s.IsActive, s.Password, s.Role, owner,
	).Scan(&s.ID, &s.CreatedAt)

	if s.OwnerID == nil {
		if _, err := r.pool.Exec(ctx,
			`UPDATE students SET owner_id = id WHERE id = $1 AND owner_id IS NULL`, s.ID); err != nil {
			return model.Student{}, fmt.Errorf("menetapkan pemilik data: %w", err)
		}
		oid := s.ID
		s.OwnerID = &oid
	}
...
func (r *studentPostgresRepository) Update(ctx context.Context, s model.Student) (model.Student, error) {
	err := r.pool.QueryRow(ctx,
		`UPDATE students SET nim = $1, name = $2, grade = $3, is_active = $4
		 WHERE id = $5
		 RETURNING id, nim, name, grade, password, role, owner_id, is_active, created_at`,
